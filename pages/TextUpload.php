...
$currentMemberId = $_SESSION['user_id'];
...
elseif ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_FILES["textfile"])) {
    ...
    if ($is_valid) {
        ...
        if (move_uploaded_file($tmp_name, $target_file)) {
            // Insert into Texts(title, author, uploader, filename, date_published)
            $stmt = $conn->prepare("
                INSERT INTO Texts (title, author, uploader, filename, date_published)
                VALUES (?, ?, ?, ?, ?)
            ");
            // uploader is INT, others are strings
            $stmt->bind_param("ssiss", $title, $author, $uploader, $safe_name, $date_published);
            $stmt->execute();
            $stmt->close();
...
<div class="section-container" style="padding: var(--navHeight); display: flex; flex-direction: column;height:100%;">
    <div class="left">
        ...
        <form action="TextUpload.php" method="post" enctype="multipart/form-data" style="margin: auto;width: fit-content;">            <label>Title:</label><br>
            ...
            <label>Uploader (your name):</label><br>
