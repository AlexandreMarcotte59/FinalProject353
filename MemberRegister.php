...
// Start session (for showing generated token)
session_start();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || empty($email) || empty($org) || empty($addr)) {
        ...
        // --- Get a new unique user_id ---
        $stmt = $pdo->query("SELECT COALESCE(MAX(user_id), 0) + 1 AS next_id FROM Members");
        $row = $stmt->fetch();
        $user_id = (int)$row['next_id'];

         // Send data to the target URL
        $url = "https://mvc353.encs.concordia.ca/MemberRegister";
